...
def corrige_com_u(df, CORRECTIFS_dict):
    ...
    for c in CORRECTIFS_dict[VAR]:
        if c['ANNEE']!= '' :
            if c['COMPOS']!= '' :
                if c['IN'] == "' '":
                    df.loc[((df[VAR] == '')|(df[VAR] is np.nan)) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                else:
                    df.loc[(df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
            else:
                if c['IN'] == "' '":
                    df.loc[((df[VAR] == '')|(pd.isna(df[VAR])) |(df[VAR]==None)) & (df['FINECOLE']==c['FINECOLE']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                else:
                    df.loc[(df['FINECOLE']==c['FINECOLE']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
        else:
            if c['COMPOS']!= '' :
...
def corrige_com_m(df, CORRECTIFS_dict):
    ...
    for c in CORRECTIFS_dict[VAR]:
        if c['ANNEE']!= '' :
            if c['COMPOS']!= '' :
                if c['IN'] == "' '":
                    df.loc[((df[VAR] == '')|(pd.isna(df[VAR])) |(df[VAR]==None)) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                elif c['ETABLI']!= '' :
                    if c['FORMAT']!= '' :
                        df.loc[(df['FORMAT']==c['FORMAT'])& (df['ETABLI']==c['ETABLI']) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                    else:
                        df.loc[(df['ETABLI']==c['ETABLI']) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                elif c['DIPLOM']!= '' :
                    df.loc[(df['DIPLOM']==c['DIPLOM']) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                elif c['COM_U'] == "' '" :
                    df.loc[((df['COM_M'] == 'nan')|(df['COM_M'] == '')|(df['COM_M'] is np.nan)) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                else:
                    if c['OUT'] == 'com_u':
                        df.loc[(df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=df.loc[(df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), 'COM_U']
                    else:
                        df.loc[(df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
            elif c['ETABLI']!= '' :
    ...
    for c in CORRECTIFS_dict[VAR]:
        if c['ANNEE']!= '' :
            if c['COMPOS']!= '' :
                if c['IN'] != "''":
                    if c['FORMAT'] != '':
                        df.loc[(df[VAR] == c['IN']) & (df['FORMAT']==c['FORMAT']) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                    else:
                        df.loc[(df[VAR] == c['IN']) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                if c['DIPLOM'] != '':
                    df.loc[(df['DIPLOM']==c['DIPLOM']) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                else:
                    df.loc[(df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                ...
                if c['DIPLOM'] != '':
                    df.loc[(df[VAR] == c['IN']) & (df['DIPLOM']==c['DIPLOM']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                elif c['FORMAT'] != '':
                    df.loc[(df[VAR] == c['IN']) & (df['FORMAT']==c['FORMAT']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                elif c['CURSUS_LMD'] != '':
                    df.loc[(df[VAR] == c['IN']) & (df['CURSUS_LMD']==c['CURSUS_LMD']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                else:
                    df.loc[(df[VAR] == c['IN']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
        else:
            df.loc[(df['COMPOS']==c['COMPOS']), VAR]=c['OUT']
    return df
...
def corrige_dc(df, CORRECTIFS_dict):
    ...
    print(VAR)
    for c in CORRECTIFS_dict[VAR]:
        df.loc[(df['RENTREE']==c['ANNEE']), VAR]=int(c['OUT'])
    return df
...
def corrige_etabli2(df, CORRECTIFS_dict):
    ...
    for c in CORRECTIFS_dict[VAR]:
        if c['COMPOS']!= '' :
            ...
            if c['ACA_U']!= '' :
                df.loc[(df['ACA_U']==c['ACA_U']) & (df['FORMAT']==c['FORMAT']) & (df['ETABLI']==c['ETABLI']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
            else:
                df.loc[(df['FORMAT']==c['FORMAT']) & (df['ETABLI']==c['ETABLI']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
    return df
...
def corrige_format(df, CORRECTIFS_dict):
    ...
    for c in CORRECTIFS_dict[VAR]:
        if c['ANNEE']!= '' :
            if c['SECT']!= '' :
                    df.loc[(df[VAR] == c['IN']) & (df['SECT']==c['SECT']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
            elif c['COMPOS']!= '' :
                if c['IN']!= '' :
                    df.loc[(df[VAR] == c['IN']) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                elif c['DIPLOM']!= '' :
    ...
    for c in CORRECTIFS_dict[VAR]:
        if c['ANNEE']!= '' :
            if c['ETABLI']!= '' :
                ...
                df.loc[((df[VAR] == '')|(df[VAR] is np.nan)) & (df['TYP_DIPL']==c['TYP_DIPL']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
            elif c['DIPLOM']!= '' :
                df.loc[((df[VAR] == '')|(df[VAR] is np.nan)) & (df['DIPLOM'] == c['DIPLOM']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
            else:
                if c['IN']== "' '" :
                    df.loc[((df[VAR] == '')|(df[VAR] is np.nan)) & (df['COMPOS']==c['COMPOS']) & (df['RENTREE']==c['ANNEE']), VAR]=c['OUT']
                else:
